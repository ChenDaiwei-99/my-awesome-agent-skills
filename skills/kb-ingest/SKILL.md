---
name: kb-ingest
description: Ingest one external source at a time into an Obsidian LLM knowledge base by creating source markdown records for PDFs, literature, GitHub repositories, web pages, or text files. Use when adding raw data to knowledge-base/raw, summarizing a PDF into knowledge-base/sources, documenting a GitHub repo with preserved links and commit context, or updating the LLM wiki after ingestion.
---
...
5. Create exactly one markdown record under the matching `knowledge-base/sources/` folder using [source-record-template.md](references/source-record-template.md).
