...
- `knowledge-base/raw/`, `knowledge-base/wiki/`, and `personal-notes/` exist.
...
- Project `paper-survey.md` files link to knowledge-base raw-source records instead of duplicating long summaries.
...
- Each raw-source record has `type: raw-source`.
- `source_kind` is one of `pdf`, `github`, `web`, or `other`.
- Required sections exist: `Summary`, `Key Takeaways`, `Keywords and Concepts`, `Important Details`, `Open Questions`, `Links Into Wiki`.
- Important external URLs are preserved.
- Local paths, repository URLs, commits, DOIs, or page locators are included when known.

## Wiki Pages

- Synthesis pages cite raw-source records or human notes.
- Concept and entity pages link back to relevant raw-source records.
...
- Stale claims with newer raw-source records are flagged, not overwritten.
...
- Missing backlinks from raw-source records to wiki pages are suggested.
